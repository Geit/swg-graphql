...
/**
 * Returns an SWG CRC32.
 *
 * @param input String to return CRC for
 * @returns Int32 CRC of the input
 */
export const getStringCrc = (input: string): number => {
  let crc = CRC_INIT;
  if (input != null) {
    for (let i = 0; i < input.length; ++i) {
      const arrayIndex = ((crc >> 24) ^ (0xff & input.charCodeAt(i))) & 0xff;
      crc = crcArray[arrayIndex] ^ (crc << 8);
    }
  }
  return crc ^ CRC_INIT;
};

export default getStringCrc;
